...
export const GET: APIRoute = async ({ params, request }) => {
  ...
  const rows = await loadModelLeaderboards();
  const groups = localizeModelLeaderboardGroups(buildModelLeaderboardGroups(rows), messages);
  const currentGroup = groups.find(group => group.taskSlug === params.taskSlug);
